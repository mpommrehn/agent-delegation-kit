...
|---|---|---|---|
| `scanner` | sonnet | Logs, CSVs, telemetry, unfamiliar directories. Conclusions only. | Read-only tool list, turn budget, skips `CLAUDE.md` |
| `browser-checker` | sonnet | Open a page, check a written list, report a paragraph. | No Write or Edit, turn budget, skips `CLAUDE.md` |
| `executor` | sonnet | Mechanical, specified, tested work. | Git worktree isolation, turn budget, evidence-file contract |
| `reviewer` | inherit | Judging a diff, a plan or an evidence file. | No Write or Edit. Deliberately stays on the parent's model |
...
- **`scanner` is read-only by instruction, not by construction.** It needs a
  shell for `awk` and `wc`, and a shell can write. Its tool list withholds
  Write and Edit. The rest is the prompt.
...
in writing, has a turn budget and carries stop-losses; that the read-only types
cannot reach the write tools; that `install.sh` is idempotent, never clobbers a
local edit without `--force`, and writes nothing in `--check` or `--dry-run`;
and that no tracked file carries a local path, an IP address or an email
address.
